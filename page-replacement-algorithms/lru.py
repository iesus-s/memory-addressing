    ...
    for counter in range(len(text_input)):
        ...
        if full == 32 and found == 0:
            ...
            for loop in range(32):
                # IF NEW larger than or equal to MAIN
                if lifo <= main_memory[loop]["Reference Bit"]:
                    # IF BOTH PAGES have the SAME VALUE
                    if lifo == main_memory[loop]["Reference Bit"]:
                        # Check if BOTH are DIRTY
                        if main_memory[lifo]["Dirty Bit"] == 1:
                            if main_memory[loop]["Dirty Bit"] == 1:
                                lifo = lifo
                            # If MAIN DIRTY and NEW CLEAN
                            else:
                                lifo = loop
                        # Check if BOTH are CLEAN
                        if main_memory[lifo]["Dirty Bit"] == 0:
                            if main_memory[loop]["Dirty Bit"] == 0:
                                lifo = lifo
                            # If NEW DIRTY and MAIN CLEAN
                            else:
                                lifo = lifo
                        fifo = lifo
                    else:
